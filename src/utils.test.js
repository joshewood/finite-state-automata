...
test('assert transition destination state', () => {
  const happyInput = getTransitionDestinationState(FSA.STATES[FSA.INITIAL_STATE_INDEX], FSA.INPUTS[0]);
...
test('basic final state label assurance', () => {
  const happyInput = getFinalLabelForState(FSA.STATES[FSA.INITIAL_STATE_INDEX]);
  expect(happyInput).toBe(FSA.STATE_LABELS[FSA.STATES[FSA.INITIAL_STATE_INDEX]]);
